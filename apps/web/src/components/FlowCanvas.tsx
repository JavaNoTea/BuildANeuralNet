              ...
              <div className="flex items-center gap-4">
                ...
                <div className="flex gap-2">
                  ...
                  <button
                    onClick={async () => {
                      if (!isAuthenticated) {
                        setConsoleOutput('Error: Please log in to run training jobs.\n');
                        return;
                      }

                      setConsoleOutput('Preparing training job...\n');
                      
                      try {
                        // Get current credit balance
                        const balanceResponse = await api.getCreditBalance();
                        setConsoleOutput(prev => prev + `Current credit balance: ${balanceResponse.balance} credits\n`);
                        
                        // Extract training configuration from nodes
                        const allModelNodes = modelNodes;
                        const allTrainingNodes = trainingNodes;
                        
                        // Find dataset and training config nodes
                        const datasetNode = allTrainingNodes.find(n => n.data.registryKey === 'input.dataset');
                        const trainingConfigNode = allTrainingNodes.find(n => n.data.registryKey === 'training.config');
                        
                        if (!datasetNode) {
                          setConsoleOutput(prev => prev + 'Error: No dataset node found. Please add a dataset in training mode.\n');
                          return;
                        }
                        
                        // Extract configuration
                        const datasetName = datasetNode.data.params.dataset || 'torchvision.datasets.MNIST';
                        const dataset = datasetName.split('.').pop() || 'MNIST';
                        
                        // Generate the actual PyTorch code
                        const generatedCode = generatePyTorchCode();
                        ...
                        const trainingConfig = {
                          model_data: JSON.stringify({
                            nodes: allModelNodes,
                            edges: modelEdges,
                            generated_code: generatedCode  // Include the generated code
                          }),  // Add model data to training config
                          dataset_name: dataset,
                          epochs: trainingConfigNode?.data.params.epochs || 10,
                          batch_size: trainingConfigNode?.data.params.batch_size || 32,
                          learning_rate: 0.001, // Default learning rate
                          gpu_type: 'T4'
                        };
                        
                        // Estimate training time first
                        setConsoleOutput(prev => prev + 'Estimating training time...\n');
                        try {
                          const timeEstimate = await api.estimateTrainingTime(trainingConfig);
                          // Add 50 seconds for imports and booting time
                          const adjustedTimeSeconds = timeEstimate.estimated_time_seconds + 50;
                          const adjustedTimeMinutes = adjustedTimeSeconds / 60;
                          
                          if (adjustedTimeMinutes < 1) {
                            setConsoleOutput(prev => prev + `⏱️ Estimated training time: ${Math.round(adjustedTimeSeconds)} seconds\n`);
                          } else if (adjustedTimeMinutes < 60) {
                            setConsoleOutput(prev => prev + `⏱️ Estimated training time: ${adjustedTimeMinutes.toFixed(1)} minutes\n`);
                          } else {
                            setConsoleOutput(prev => prev + `⏱️ Estimated training time: ${(adjustedTimeMinutes/60).toFixed(1)} hours\n`);
                          }
                          
                          setConsoleOutput(prev => prev + `ℹ️  Note: ${timeEstimate.note}\n\n`);
                        } catch (error) {
                          setConsoleOutput(prev => prev + `⏱️ Estimated training time: ~1-2 minutes\n`);
                          setConsoleOutput(prev => prev + `ℹ️  Note: Training logs will appear when training completes (not real-time)\n\n`);
                        }
                        
                        // Estimate cost
                        setConsoleOutput(prev => prev + 'Estimating training cost...\n');
                        const costEstimate = await api.estimateTrainingCost(trainingConfig);
                        setConsoleOutput(prev => prev + `Estimated cost: ${costEstimate.estimated_cost} credits\n`);
                        
                        if (!costEstimate.sufficient_credits) {
                          setConsoleOutput(prev => prev + 'Error: Insufficient credits for this training job.\n');
                          return;
                        }
                        
                        setConsoleOutput(prev => prev + 'Starting training job...\n');
                        
                        const job = await api.startTraining(trainingConfig.model_data, trainingConfig);
                        
                        setConsoleOutput(prev => prev + `Training job created successfully!\n`);
                        setConsoleOutput(prev => prev + `Job ID: ${job.id}\n`);
                        setConsoleOutput(prev => prev + `Status: ${job.status}\n`);
                        setConsoleOutput(prev => prev + `Estimated cost: ${job.estimated_cost} credits\n`);
                        setConsoleOutput(prev => prev + `GPU type: ${job.gpu_type}\n\n`);
                        setConsoleOutput(prev => prev + 'Monitoring training progress...\n');
                        
                        // Start polling for job status updates
                        const basePollInterval = 1000; // Poll every 1 second during training
                        const slowPollInterval = 3000; // Poll every 3 seconds when waiting
                        ...
                        let lastLogLength = 0; // Track logs to only show new ones
                        
                        // Setup polling state management
                        ...
                        const pollJobStatus = async () => {
                          ...
                          if (!pollingRef.current.isActive || pollAttempts >= maxPollAttempts || consecutiveAuthErrors >= maxConsecutiveAuthErrors) {
                            return;
                          }
                          ...
                          try {
                            ...
                            // Reset auth errors on successful request
                            ...
                            const [updatedJob, logsResponse] = await Promise.all([
                              api.getTrainingJob(job.id),
                              api.getTrainingLogs(job.id)
                            ]);
                            
                            // Update polling interval based on status
                            if (updatedJob.status === 'running') {
                              pollInterval = basePollInterval; // Fast polling during training
                            } else {
                              pollInterval = slowPollInterval; // Slower when pending/waiting
                            }
                            
                            // Display new logs if available
                            if (logsResponse.logs && logsResponse.logs.length > lastLogLength) {
                              const newLogs = logsResponse.logs.substring(lastLogLength);
                              if (newLogs.trim()) {
                            ...
                            // Only update console if status changed
                            if (updatedJob.status !== lastStatus) {
                              setConsoleOutput(prev => prev + `[${timestamp}] Status: ${updatedJob.status}\n`);
                              lastStatus = updatedJob.status;
                              
                              if (updatedJob.status === 'running') {
                                setConsoleOutput(prev => prev + `[${timestamp}] Training is now running on GPU...\n`);
                              } else if (updatedJob.status === 'completed') {
                                setConsoleOutput(prev => prev + `[${timestamp}] Training completed successfully!\n`);
                                if (updatedJob.actual_cost) {
                                  setConsoleOutput(prev => prev + `[${timestamp}] Actual cost: ${updatedJob.actual_cost} credits\n`);
                                }
                                if (updatedJob.results) {
                                  setConsoleOutput(prev => prev + `[${timestamp}] Results:\n${updatedJob.results}\n`);
                                }
                                setConsoleOutput(prev => prev + `[${timestamp}] Training job finished. You can find your trained model in the models section.\n`);
                                pollingRef.current.cleanup(); // Stop polling
                                return;
                              } else if (updatedJob.status === 'failed') {
                                setConsoleOutput(prev => prev + `[${timestamp}] Training failed!\n`);
                                if (updatedJob.error_message) {
                                  setConsoleOutput(prev => prev + `[${timestamp}] Error: ${updatedJob.error_message}\n`);
                                }
                                pollingRef.current.cleanup(); // Stop polling
                                return;
                              } else if (updatedJob.status === 'cancelled') {
                                setConsoleOutput(prev => prev + `[${timestamp}] Training was cancelled.\n`);
                                pollingRef.current.cleanup(); // Stop polling
                                return;
                              }
                            }
                            
                            // Continue polling if job is still running and we haven't hit limits
                            if (pollingRef.current.isActive && ['pending', 'running'].includes(updatedJob.status) && pollAttempts < maxPollAttempts) {
                              pollingRef.current.timeoutId = setTimeout(pollJobStatus, pollInterval);
                            } else if (pollAttempts >= maxPollAttempts) {
                              setConsoleOutput(prev => prev + `[${timestamp}] Polling timeout reached. Job may still be running.\n`);
                              pollingRef.current.cleanup();
                            }
                            
                          } catch (pollError: any) {
                            if (!pollingRef.current.isActive) {
                              return; // Stop processing if polling was disabled
                            }
                            ...
                            if (pollError && (pollError.status === 401 || pollError.status === 403)) {
                              consecutiveAuthErrors++;
                              setConsoleOutput(prev => prev + `[${timestamp}] Authentication error (${consecutiveAuthErrors}/${maxConsecutiveAuthErrors}). Retrying...\n`);
                              
                              if (consecutiveAuthErrors >= maxConsecutiveAuthErrors) {
                                setConsoleOutput(prev => prev + `[${timestamp}] Too many authentication errors. Please refresh and try again.\n`);
                                pollingRef.current.cleanup();
                                return;
                              }
                            } else if (pollError && pollError.message && pollError.message.includes('timeout')) {
                              setConsoleOutput(prev => prev + `[${timestamp}] Request timeout. Retrying with longer delay...\n`);
                            } else if (pollError && pollError.message && pollError.message.includes('Failed to fetch')) {
                              setConsoleOutput(prev => prev + `[${timestamp}] Network error. Retrying...\n`);
                            } else {
                              // For other errors, log but continue polling with backoff
                              let errorMessage = 'Unknown error';
                              if (pollError && typeof pollError === 'object') {
                                if (pollError.detail) {
                                  errorMessage = pollError.detail;
                                } else if (pollError.message) {
                                  errorMessage = pollError.message;
                                } else {
                                  try {
                                    errorMessage = JSON.stringify(pollError);
                                  } catch {
                              ...
                              setConsoleOutput(prev => prev + `[${timestamp}] Polling error: ${errorMessage}. Retrying...\n`);
                            }
                            
                            // Continue polling unless we've hit limits
                            if (pollingRef.current.isActive && pollAttempts < maxPollAttempts && consecutiveAuthErrors < maxConsecutiveAuthErrors) {
                              const retryDelay = pollError && pollError.message && pollError.message.includes('timeout') ? 5000 : pollInterval * 2;
                              pollingRef.current.timeoutId = setTimeout(pollJobStatus, retryDelay);
                            } else {
                              setConsoleOutput(prev => prev + `[${timestamp}] Stopping polling due to repeated errors or limits reached.\n`);
                              pollingRef.current.cleanup();
                            }
                          }
                        };
